...
def _device_location_from_status(status: dict[str, str]) -> str | None:
    """Find the best location value from the status."""
    return status.get("device.location")
...
class PyNUTData:
    ...
    def _get_device_info(self) -> NUTDeviceInfo | None:
        ...
        device_location = _device_location_from_status(self._status)
